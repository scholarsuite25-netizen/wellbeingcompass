...
export function mapArticle(db:any){
  ...
  return {
    ...
    author: {
      slug: db.author.slug, name: db.author.name, role: db.author.role, bio: db.author.bio, avatar: db.author.avatar,
      isFounder: db.author.isFounder ?? false,
      credentials: db.author.credentials ?? undefined,
      professionalTitle: db.author.professionalTitle ?? undefined,
      currentPosition: db.author.currentPosition ?? undefined,
      shortBio: db.author.shortBio ?? undefined,
      fullBio: db.author.fullBio ?? undefined,
      profilePhoto: db.author.profilePhoto ?? undefined,
      specializations: parseJson<string[]>(db.author.specializations, []),
      expertise: parseJson<any[]>(db.author.expertise, []),
      education: parseJson<any[]>(db.author.education, []),
      fellowships: parseJson<string[]>(db.author.fellowships, []),
      certifications: parseJson<string[]>(db.author.certifications, []),
      awards: parseJson<string[]>(db.author.awards, []),
      researchInterests: parseJson<string[]>(db.author.researchInterests, []),
      socialLinks: parseJson<any[]>(db.author.socialLinks, []),
    },
